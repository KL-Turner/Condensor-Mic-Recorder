function [S,f,Serr]=mtspectrumc_CM(data,params)
...
[f,findx]=getfgrid_CM(Fs,nfft,fpass); 
tapers=dpsschk_CM(tapers,N,Fs); % check tapers
J=mtfftc_CM(data,tapers,nfft,Fs);
J=J(findx,:,:);
S=squeeze(mean(conj(J).*J,2));
if trialave; S=squeeze(mean(S,2));end;
if nargout==3; 
   Serr=specerr_CM(S,J,err,trialave);
end;
