function [S,t,f,Serr] = mtspecgramc_CM(data,movingwin,params)
...
[tapers,pad,Fs,fpass,err,trialave,params]=getparams_CM(params);
...
data=change_row_to_column_CM(data);
[N,Ch]=size(data);
Nwin=round(Fs*movingwin(1)); % number of samples in window
Nstep=round(movingwin(2)*Fs); % number of samples to step through
nfft=max(2^(nextpow2(Nwin)+pad),Nwin);
f=getfgrid_CM(Fs,nfft,fpass); Nf=length(f);
params.tapers=dpsschk_CM(tapers,Nwin,Fs); % check tapers
...
for n=1:nw;
   ...
   datawin=data(indx,:);
   if nargout==4
     [s,f,serr]=mtspectrumc_CM(datawin,params);
     Serr(1,n,:,:)=squeeze(serr(1,:,:));
     Serr(2,n,:,:)=squeeze(serr(2,:,:));
   else
     [s,f]=mtspectrumc_CM(datawin,params);
   end
   S(n,:,:)=s;
end;
S=squeeze(S); 
